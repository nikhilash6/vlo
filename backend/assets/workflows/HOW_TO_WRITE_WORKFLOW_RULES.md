...
[README.md](../../services/gen_pipeline/README.md).
...
- `backend/assets/workflows/` — **primary**. User-authored workflows live
  here. Sidecars sit next to each workflow as
  `<stem>.rules.json`.
...
When editing a packaged workflow in the app, saving produces a user copy in
...
uses manual widget exposure, hiding, cross-node widget grouping, a derived
widget, a frontend control with a conditional graph rewrite, and both
pipeline stages.
...
- **`705` and `714` are bypass switches** driven by the `retake_mode`
  ...
  their value.
- **Widget grouping is cross-node**: `noise_seed` on `115`, `FPS` on `214`,
  and `Max Pixel Size` on `497` all share `group_id: "video_generation"`
  and render under one collapsible section, ordered by `group_order`.
  `214`'s FPS widget is hidden but its `group_order` still contributes to
  group placement.
- **`243.noise_seed` is hidden globally** rather than scoped to a group —
  the upscale seed is not exposed at all.
- **`prompt_enhancer_enabled` is a frontend control** (not bound to any
  node widget). The `rewrites` entry bypasses node `599` (the enhancer
  branch) whenever that toggle is off.
...
author is always really asking is: _which widgets should the user see, and
with what labels / ranges / grouping?_
...
`when` is _dynamic_: it is re-evaluated whenever panel state changes (an
...
A stage can declare **multiple targets sharing the same `source`** when the
same mask drives different downstream consumers — e.g. one target with
`purpose: "video"` (drives the mask_crop output) and a second with
`purpose: "audio_timing"` and an explicit `render_fps` (drives audio
timing extraction).
...
This is the right pattern for workflows where a mask is _accepted but not
mandatory_ — for example IC-edit, where the model can run on the full
...
(w, h) feeds into the aspect-aware mask crop geometry. The runtime
enforces this dependency even when `after` is omitted, but author it
explicitly so the intent is visible in the sidecar.
...
- `bypass` node ids — the listed nodes are ComfyUI-bypassed at dispatch
  time, which prunes them from the executed graph.
...
The practical pattern is to assign each group a numeric _band_ and place
